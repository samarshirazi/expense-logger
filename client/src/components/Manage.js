...
function Manage({ expenses = [], budgets = {}, onBudgetUpdate, onGoalUpdate }) {
  ...
  const [newGoal, setNewGoal] = useState({ title: '', target: '', type: 'savings', icon: '💰' });
  ...
  const handleBudgetEdit = (category, newBudget) => {
    ...
    setCategoryBudgets(prev => ({
      ...prev,
      [category]: parsedBudget
    }));
    setEditingBudget(null);
  };

  const handleAISuggestBudget = () => {
    // Calculate suggested budgets based on recent spending with 10% buffer
    const suggestedBudgets = {};
    Object.keys(CATEGORY_META).forEach(category => {
      const avgSpending = monthlySpending[category] || 100;
      suggestedBudgets[category] = Math.round(avgSpending * 1.15);
    });
  ...
  const handleAddGoal = () => {
    ...
    setNewGoal({ title: '', target: '', type: 'savings', icon: '💰' });
    setShowAddGoal(false);
  };

  const handleMarkGoalAchieved = (goalId) => {
    ...
    <div className="manage-screen">
      ...
      <div className="manage-header">
        <div className="manage-header-top">
          ...
          >
            <span className="ai-icon">💬</span> AI Suggest Budget
          </button>
        </div>
        <select
                ...
                <div key={category} className="category-budget-card">
                  <div className="category-budget-header">
                    <div className="category-info">
                      <span className="category-icon-badge" style={{ backgroundColor: meta.color + '20', color: meta.color }}>
                        {meta.icon}
                      </span>
                ...
                <div key={goal.id} className="goal-card">
                  ...
                  <div className="goal-content">
                    ...
                    <div className="goal-footer">
                      ...
                      {progress >= 100 ? (
                        <button
                          className="goal-action-btn achieved"
        ...
        <div className="modal-overlay" onClick={() => setShowAISuggest(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="modal-description">
              Based on your spending patterns, we recommend the following budgets with a 15% buffer:
            </p>
              ...
              {Object.keys(CATEGORY_META).map(category => (
                <div key={category} className="suggested-budget-row">
                  ...
                  <span className="suggested-amount">
                    {formatCurrency(Math.round((monthlySpending[category] || 100) * 1.15))}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="goal-form">
              <div className="form-group">
                ...
                <select
                  value={newGoal.type}
                  onChange={(e) => setNewGoal({ ...newGoal, type: e.target.value, icon: e.target.value === 'savings' ? '💰' : '🎯' })}
                >
                  <option value="savings">Savings Goal</option>
                  <option value="reduction">Spending Reduction</option>
                </select>
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                <label>Target {newGoal.type === 'savings' ? 'Amount ($)' : 'Percentage (%)'}</label>
                <input
                  type="number"
                  placeholder={newGoal.type === 'savings' ? '500' : '15'}
            ...
            <div className="modal-actions">
              <button className="modal-btn cancel" onClick={() => setShowAddGoal(false)}>
                Cancel
              </button>
              <button className="modal-btn apply" onClick={handleAddGoal}>
                Add Goal
              </button>
            </div>
          </div>
